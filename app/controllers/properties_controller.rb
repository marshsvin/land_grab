class PropertiesController < ApplicationController
  ...
  def current_user_must_be_property_user
    ...
    unless current_user == property.landowner
  ...
  def index
    ...
    @properties = @q.result(:distinct => true).includes(:landowner, :property_bookings, :reviews).page(params[:page]).per(10)
    @location_hash = Gmaps4rails.build_markers(@properties.where.not(:address_latitude => nil)) do |property, marker|
      ...
      marker.infowindow "<h5><a href='/properties/#{property.id}'>#{property.landowner_id}</a></h5><small>#{property.address_formatted_address}</small>"
    end
  ...
  def create
    ...
    @property.landowner_id = params[:landowner_id]
    ...
    @property.property_pictures = params[:property_pictures]
